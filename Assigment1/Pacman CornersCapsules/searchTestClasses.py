...
class CornerAndCapsulesHeuristicSanity(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        super(CornerAndCapsulesHeuristicSanity, self).__init__(question, testDict)
        self.layout_text = testDict['layout']
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        start_state = problem.getStartState()
        h0 = searchAgents.cornersAndCapsulesHeuristic(start_state, problem)
        succs = problem.getSuccessors(start_state)
        # cornerConsistencyA
        for succ in succs:
        ...
        heuristics = []
        for state in states:
            heuristics.append(searchAgents.cornersAndCapsulesHeuristic(state, problem))
        for i in range(0, len(heuristics) - 1):
            h0 = heuristics[i]
            h1 = heuristics[i+1]
            # cornerConsistencyB
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        start_state = pacman.GameState()
        start_state.initialize(lay, 0)
        problem = searchAgents.CornersAndCapsulesProblem(start_state)
        solution = search.astar(problem, searchAgents.cornersAndCapsulesHeuristic)
        handle.write('cost: "%d"\n' % len(solution))
        handle.write('path: """\n%s\n"""\n' % wrap_solution(solution))
        handle.close()
        return True
...
class CornerAndCapsulesHeuristicPacman(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        super(CornerAndCapsulesHeuristicPacman, self).__init__(question, testDict)
        self.layout_text = testDict['layout']
