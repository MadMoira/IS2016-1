...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        believed_pos = self.wallBeliefs.argMax()
        print("Creencia")
        print(believed_pos)
        temp = []
        for action in legal:
            nextLocation = Actions.getSuccessor(believed_pos, action)
            temp.append((self.distancer.getDistance(nextLocation, (1, 3)), action))
        return min(temp)[1]
