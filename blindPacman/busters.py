...
def get_prob_value(error, different):
    return ((1-error)**(4-different))*(error)**different
...
def getObservationDistributionNoisyWall(noisyWall):
    ...
    if noisyWall not in observationDistributionsWalls:
        ...
        for value in WALL_VALUES:
            different = 0
            for wall in xrange(0, 4):
                if value[wall][1] != noisyWall[wall][1]:
                    different += 1
            distribution[value] = get_prob_value(EPSILON, different)
        observationDistributionsWalls = distribution
    # import pprint
    # print("Distribution")
    # pprint.pprint(observationDistributionsWalls)

    return observationDistributionsWalls
