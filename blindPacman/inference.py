...
class ExactInference(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for p in self.legalPositions:
            self.beliefs[p] = 1.0/len(self.legalPositions)
    ...
    def observe(self, gameState, observationWalls):
        ...
        *unknown* position of the pacman.
        ...
        """
        import pprint
        noisyPerceptions = observationWalls
        emissionModel = busters.getObservationDistributionNoisyWall(noisyPerceptions)

        allPossible = util.Counter()
        for location in self.legalPositions:
            # print("Ubicacion")
            # print(location)
            # print("True perceptions")
            # print(gameState.getTrueWallPerception(location))
            trueWallPerceptions = gameState.getTrueWallPerception(location)
            # pprint.pprint(self.beliefs[location])
            # pprint.pprint(emissionModel[observationWalls])
            allPossible[location] = self.beliefs[location] * emissionModel[trueWallPerceptions]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.legalPositions:
            newPostDist = self.getPacmanSuccesorDistribution(gameState, oldPos)
            for newPos in newPostDist:
                allPossible[newPos] += self.beliefs[oldPos] * newPostDist[newPos]
            # print(newPostDist)
        allPossible.normalize()
        self.beliefs = allPossible
